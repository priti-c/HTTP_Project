//Basic HTTP server : Task3 : Serve different response based on url path.
...
int main() {
    ...
    while (1) {
        ...
        // Step 9: Choose HTML body based on path
        char body[1024];
        if (strcmp(path, "/hello") == 0) {
            snprintf(body, sizeof(body), "<h1>Hello Page</h1>");
        } else if (strcmp(path, "/bye") == 0) {
            snprintf(body, sizeof(body), "<h1>Goodbye Page</h1>");
        } else {
            snprintf(body, sizeof(body), "<h1>Default Page</h1>");
        }

        // Step 10: Append time & IP to HTML
        char content[2048];
        snprintf(content, sizeof(content),
                 "<html><body>%s<p>Time: %s</p><p>Client IP: %s</p></body></html>",
                 body, time_str, client_ip);

        // Step 11: Build full HTTP response
        char response[4096];
        ...
        // Step 12: Send response & close connection
